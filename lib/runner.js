...
var elmCoverageSrcDir = (args) => {
  return path.join(coverageDir, args.rootPath, "elm-coverage-src");
};
...
var sourceDirectories = args => {
    var elmJsonPath = path.join(args.rootPath, 'elm.json');
...
var fixOutOfRootPath = path => {
  // TODO this is too simplistic (we handle ../x but not foo/../../x nor many other cases)
...
var generateReport = (log, args) => () => {
    ...
    return aggregate(args.rootPath)
        .then(function(data) {
