...
var setupTests = (log, args) => () => {
    ...
    return fs
        ...
        .then(function(elmPackage) {
            ...
            // Packages don't have a source-directories property. For
            // applications, especially with multiple source directories, we
            // want to resolve all directories - other than the one we're
            // instrumenting - to an absolute path.
            if (elmPackage["source-directories"]) {
                elmPackage["source-directories"] = elmPackage["source-directories"].map(function(item) {
                    if (path.resolve(item) == originalPath) { return item; }
    ...
    return allSources(args)
        ...
        .then(function() {
            ...
            return new Promise(function(resolve, reject) {
                ...
                // I do not know why, but we have to consume this?
