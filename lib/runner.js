...
var fakeElmBinary = path.join(__dirname, "..", "bin", "fake-elm");
...
var setupTests = (log, args) => () => {
    ...
    return fs
        ...
        .then(function(elmPackage) {
			log.debug("modifyingTests", "Generating elm.json for coverage at " + tmpElmJson + "...");
            ...
            elmPackage["source-directories"] = elmPackage[
                ...
                if (path.resolve(item) == originalPath) {
                    return item;
                } else {
					...
					return path.relative(path.resolve(coverageDir), path.resolve(item));
                }
            });
			elmPackage["source-directories"].push(
				path.relative(path.resolve(coverageDir), path.join( __dirname, "..", "kernel-src" ) )
			);
        ...
        .then(function() {
            ...
            log.debug("copyTests", "Copying tests from " + args.tests + " to " +  generatedTestsDir);
...
var cleanup = (log, args) => () => {
	...
	log.error("was there errors?", args);
};
        ...
        .then(function() {
            ...
            return new Promise(function(resolve, reject) {
                ...
                process.on("exit", function(code) {
					log.debug("", "finished instrumenting" + code);
                    if (code === 0) {
                        resolve();
                    } else {
                        reject(err);
