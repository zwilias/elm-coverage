...
var elmTestGeneratedDir = path.join(".coverage","instrumented","elm-stuff","generated-code","elm-explorations","test");
        ...
        .spread(function(fileList, infoData) {
            return Promise.reduce(
                fileList.filter(isCoverageDataFile).map(function(fileName) {
                    return readJsonFile(path.join(elmTestGeneratedDir, fileName))
                }),
                addCoverage,
                infoData
            );
